    ...
    <div className="container mx-auto px-6 flex justify-between items-center">
      ...
      <div className="hidden md:flex items-center space-x-8">
        {isHomePage ? <>
          ...
          {localStorage.getItem('accessToken') ? <Link to="/dashboard" className={`px-5 py-2 rounded-full font-medium transition-all duration-300 text-sm shadow-md hover:shadow-lg transform hover:-translate-y-1 ${isScrolled || !isHomePage ? 'bg-gradient-to-r from-ocean to-ocean-light text-white hover:from-ocean-dark hover:to-ocean' : 'glassmorphism text-white hover:bg-white/30'}`} onClick={handleGetStarted}>
            Get Started
          </Link> : <Link to="/login" className={`px-5 py-2 rounded-full font-medium transition-all duration-300 text-sm shadow-md hover:shadow-lg transform hover:-translate-y-1 ${isScrolled || !isHomePage ? 'bg-gradient-to-r from-ocean to-ocean-light text-white hover:from-ocean-dark hover:to-ocean' : 'glassmorphism text-white hover:bg-white/30'}`} onClick={handleGetStarted}>
            Get Started
          </Link>}
          ...
          {location.pathname === '/login' || location.pathname === '/signup' || (
            <>
              ...
              <Link to="/" className={`px-5 py-2 rounded-full font-medium transition-all duration-300 text-sm shadow-md hover:shadow-lg transform hover:-translate-y-1 ${isScrolled || !isHomePage ? 'bg-gradient-to-r from-ocean to-ocean-light text-white hover:from-ocean-dark hover:to-ocean' : 'glassmorphism text-white hover:bg-white/30'}`} onClick={handleLogout}>
                Logout
              </Link>
            </>
          )}
      ...
      <div className="container mx-auto px-6 pt-4 pb-8 space-y-4 glassmorphism backdrop-blur-xl bg-white/90 dark:bg-gray-900/90">
        {isHomePage ? <>
          ...
          <div className="flex flex-col space-y-2 mt-4">
            <Link to="/login" className="bg-gradient-to-r from-ocean to-ocean-light text-white px-5 py-2 rounded-full shadow-md inline-block text-center" onClick={() => setIsMobileMenuOpen(false)}>
              Get Started
            </Link>
          </div>
        </> : <>
          {location.pathname === '/login' || location.pathname === '/signup' ?
            ...
            (
              <>
                ...
                <Link to="/" className="bg-gradient-to-r from-ocean to-ocean-light text-white px-5 py-2 rounded-full shadow-md inline-block text-center" onClick={() => setIsMobileMenuOpen(false)}>
                  logout
                </Link>
              </>
            )}
